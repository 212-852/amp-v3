...
import { CalendarDays, Paperclip, Share2 } from "lucide-react";
...
export function MessageShare({ messageUuid, attachmentCount, language, orders }: { messageUuid: string; attachmentCount: number; language: "ja" | "en"; orders: InboxOrder[] }) {
  ...
  return (
    <>
      ...
      <Modal
        label={language === "en" ? "Share message" : "メッセージを共有"}
        open={open}
        onClose={() => setOpen(false)}
        overlayClassName="adminMessageShareOverlay"
        panelClassName="adminMessageShareDialog"
      >
        <form className="adminMessageShareForm" action={submit}>
          <header><div><Share2 aria-hidden="true" /><h2>{language === "en" ? "Share message" : "メッセージを共有"}</h2></div></header>
