...
import { identityDispatcher, SESSION_COOKIE_NAME, type AnimalStatus } from "@/lib/identity";

const copy = {
  ja: { title: "動物データを新規登録", description: "検索・お客様の選択肢・公開ページで共通利用する情報を登録します。", back: "動物データベースへ戻る", basic: "基本情報", details: "公開・輸送情報", nameJa: "名称（日本語）", nameEn: "名称（英語）", tags: "タグ", slug: "URL用の名前", aliases: "別名・検索語", summaryJa: "特徴（日本語）", summaryEn: "特徴（英語）", transportJa: "輸送条件（日本語）", transportEn: "輸送条件（英語）", crateJa: "クレート目安（日本語）", crateEn: "クレート目安（英語）", image: "代表画像URL", status: "公開状態", save: "登録する", hint: "カンマ区切りで複数入力できます。例：犬、大型犬、長毛", statuses: { draft: "下書き", published: "公開中", archived: "非公開" } },
  en: { title: "Add animal record", description: "Add shared information for search, customer choices, and public pages.", back: "Back to animal database", basic: "Basic information", details: "Public & transport information", nameJa: "Japanese name", nameEn: "English name", tags: "Tags", slug: "URL slug", aliases: "Aliases / keywords", summaryJa: "Japanese profile", summaryEn: "English profile", transportJa: "Japanese transport guidance", transportEn: "English transport guidance", crateJa: "Japanese crate guidance", crateEn: "English crate guidance", image: "Representative image URL", status: "Status", save: "Save record", hint: "Separate multiple tags with commas.", statuses: { draft: "Draft", published: "Published", archived: "Archived" } },
} as const;
  ...
  return <section className="adminContentPage adminPetPage adminPetNew">
    ...
    <form action={create} className="adminPetForm">
      <fieldset><legend>{text.basic}</legend><div className="adminPetFields">
        ...
        <label>{text.tags}<input name="tags" required placeholder={language === "ja" ? "犬、大型犬、長毛" : "dog, large, long-haired"} /><small>{text.hint}</small></label>
        <label>{text.slug}<input name="slug" required pattern="[a-z0-9]+(?:-[a-z0-9]+)*" placeholder="golden-retriever" /></label>
        <label>{text.aliases}（日本語）<input name="aliasesJa" /></label><label>{text.aliases}（English）<input name="aliasesEn" /></label>
      </div></fieldset>
