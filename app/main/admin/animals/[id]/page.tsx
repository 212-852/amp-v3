...
import { animalCountryOptions, animalSizeOptions, animalStatuses, isAnimalStatus, slugify, splitCommaValues } from "@/lib/form";
    ...
    <form action={update} className="adminPetForm">
      ...
      <fieldset><legend>{label.basic}</legend><div className="adminLanguageEditor">
        ...
        <div className="adminLanguagePanels">
          <div className="adminLanguagePanel adminLanguagePanel--ja"><div className="adminPetFields"><label>動物種<input name="speciesJa" required defaultValue={animal.profile.species.ja} /></label><label>別名（日本語）<input name="aliasesJa" defaultValue={animal.aliases.ja.join("、")} /></label><label>原産国<input list="editAnimalCountriesJa" name="originJa" required defaultValue={animal.profile.origin.ja} /><datalist id="editAnimalCountriesJa">{countries.map((country) => <option key={`${country.ja}-${country.en}`} value={country.ja}>{country.en}</option>)}</datalist></label><label>サイズ区分<select name="sizeJa" required defaultValue={animal.profile.sizeClass.ja}>{!animalSizeOptions.some((size) => size.ja === animal.profile.sizeClass.ja) && animal.profile.sizeClass.ja ? <option value={animal.profile.sizeClass.ja}>{animal.profile.sizeClass.ja}</option> : null}{animalSizeOptions.map((size) => <option key={size.ja} value={size.ja}>{size.ja}</option>)}</select></label><label>体重目安<input name="weightJa" required defaultValue={animal.profile.weightGuide.ja} /></label><label>寿命目安<input name="lifespanJa" required defaultValue={animal.profile.lifespanGuide.ja} /></label><label>特徴<textarea name="traitsJa" required rows={4} defaultValue={animal.profile.traits.ja} /></label><label>推奨輸送方法<input name="transportJa" required defaultValue={animal.profile.transportMethod.ja} /></label></div></div>
          <div className="adminLanguagePanel adminLanguagePanel--en"><div className="adminPetFields"><label>Name (English)<input name="nameEn" required maxLength={80} defaultValue={animal.name.en} /><small>URL用の名前は英名から小文字・ハイフン区切りで自動生成されます。</small></label><label>Animal type<input name="speciesEn" required defaultValue={animal.profile.species.en} /></label><label>Aliases (English)<input name="aliasesEn" defaultValue={animal.aliases.en.join(", ")} /></label><label>Country of origin<input list="editAnimalCountriesEn" name="originEn" required defaultValue={animal.profile.origin.en} /><datalist id="editAnimalCountriesEn">{countries.map((country) => <option key={`${country.en}-${country.ja}`} value={country.en}>{country.ja}</option>)}</datalist></label><label>Size class<select name="sizeEn" required defaultValue={animal.profile.sizeClass.en}>{!animalSizeOptions.some((size) => size.en === animal.profile.sizeClass.en) && animal.profile.sizeClass.en ? <option value={animal.profile.sizeClass.en}>{animal.profile.sizeClass.en}</option> : null}{animalSizeOptions.map((size) => <option key={size.en} value={size.en}>{size.en}</option>)}</select></label><label>Weight guide<input name="weightEn" required defaultValue={animal.profile.weightGuide.en} /></label><label>Lifespan guide<input name="lifespanEn" required defaultValue={animal.profile.lifespanGuide.en} /></label><label>Traits<textarea name="traitsEn" required rows={4} defaultValue={animal.profile.traits.en} /></label><label>Recommended transport<input name="transportEn" required defaultValue={animal.profile.transportMethod.en} /></label></div></div>
        </div>
      </div></fieldset>
