    ...
    <form action={update} className="adminPetForm">
      ...
      <fieldset><legend>{label.basic}</legend><div className="adminLanguageEditor">
        ...
        <div className="adminLanguagePanels">
          <div className="adminLanguagePanel adminLanguagePanel--ja"><div className="adminPetFields"><label>動物種<select name="speciesJa" required defaultValue={animal.profile.species.ja}>{!animalSpeciesOptions.some((species) => species.ja === animal.profile.species.ja) && animal.profile.species.ja ? <option value={animal.profile.species.ja}>{animal.profile.species.ja}</option> : null}{animalSpeciesOptions.map((species) => <option key={species.ja} value={species.ja}>{species.ja}</option>)}</select></label><label>別名（日本語）<input name="aliasesJa" defaultValue={animal.aliases.ja.join("、")} /></label><label>原産国<select name="originJa" required defaultValue={animal.profile.origin.ja}>{!countries.some((country) => country.ja === animal.profile.origin.ja) && animal.profile.origin.ja ? <option value={animal.profile.origin.ja}>{animal.profile.origin.ja}</option> : null}{countries.map((country) => <option key={`${country.ja}-${country.en}`} value={country.ja}>{country.ja}</option>)}</select></label><label>サイズ区分<select name="sizeJa" required defaultValue={animal.profile.sizeClass.ja}>{!animalSizeOptions.some((size) => size.ja === animal.profile.sizeClass.ja) && animal.profile.sizeClass.ja ? <option value={animal.profile.sizeClass.ja}>{animal.profile.sizeClass.ja}</option> : null}{animalSizeOptions.map((size) => <option key={size.ja} value={size.ja}>{size.ja}</option>)}</select></label><label>体重目安<input name="weightJa" required defaultValue={animal.profile.weightGuide.ja} /></label><label>寿命目安<input name="lifespanJa" required defaultValue={animal.profile.lifespanGuide.ja} /></label><label>特徴<textarea name="traitsJa" required rows={4} defaultValue={animal.profile.traits.ja} /></label></div></div>
          <div className="adminLanguagePanel adminLanguagePanel--en"><div className="adminPetFields"><label>Name (English)<input name="nameEn" required maxLength={80} defaultValue={animal.name.en} /><small>URL用の名前は英名から小文字・ハイフン区切りで自動生成されます。</small></label><label>Animal type<select name="speciesEn" required defaultValue={animal.profile.species.en}>{!animalSpeciesOptions.some((species) => species.en === animal.profile.species.en) && animal.profile.species.en ? <option value={animal.profile.species.en}>{animal.profile.species.en}</option> : null}{animalSpeciesOptions.map((species) => <option key={species.en} value={species.en}>{species.en}</option>)}</select></label><label>Aliases (English)<input name="aliasesEn" defaultValue={animal.aliases.en.join(", ")} /></label><label>Country of origin<select name="originEn" required defaultValue={animal.profile.origin.en}>{!countries.some((country) => country.en === animal.profile.origin.en) && animal.profile.origin.en ? <option value={animal.profile.origin.en}>{animal.profile.origin.en}</option> : null}{countries.map((country) => <option key={`${country.en}-${country.ja}`} value={country.en}>{country.en}</option>)}</select></label><label>Size class<select name="sizeEn" required defaultValue={animal.profile.sizeClass.en}>{!animalSizeOptions.some((size) => size.en === animal.profile.sizeClass.en) && animal.profile.sizeClass.en ? <option value={animal.profile.sizeClass.en}>{animal.profile.sizeClass.en}</option> : null}{animalSizeOptions.map((size) => <option key={size.en} value={size.en}>{size.en}</option>)}</select></label><label>Weight guide<input name="weightEn" required defaultValue={animal.profile.weightGuide.en} /></label><label>Lifespan guide<input name="lifespanEn" required defaultValue={animal.profile.lifespanGuide.en} /></label><label>Traits<textarea name="traitsEn" required rows={4} defaultValue={animal.profile.traits.en} /></label></div></div>
        </div>
      </div></fieldset>
